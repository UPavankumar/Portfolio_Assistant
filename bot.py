...
class SessionCache:
    ...
    def load_cache() -> Optional[Dict]:
        try:
            ...
            if datetime.now() - cache_time > timedelta(hours=48):
                return None
...
# =========================
@st.cache_data
def extract_structured_data(resume_content: str) -> Dict:
...
def classify_intent(query: str, last_intent: Optional[str]) -> str:
    ...
    # Follow-ups
    if any(p in q for p in ['tell me more', 'what about', 'and', 'also', 'what else', 'more']) and last_intent:
        return last_intent
    
    # Keywords
    if any(w in q for w in ['work', 'job', 'company', 'employer', 'role', 'position', 'career', 'worked']):
        return 'experience'
    if any(w in q for w in ['project', 'built', 'created', 'developed', 'application', 'model']):
        return 'projects'
...
def intelligent_search(query: str, intent: str, structured: Dict) -> Tuple[str, str]:
    ...
    try:
        ...
        return f"Search error: {str(e)}", 'error'
...
if user_input := st.chat_input("Your message..."):
    ...
    mode_map = {
        ...
        "builder": "Technical details and steps."
    }
    
    # System prompt
    system_prompt = f"""You are Alfred Pennyworth, Mr. Pavan Kumar's AI assistant.
...
Answer directly."""
    ...
    if source == 'structured':
        ...
        try:
            ...
            response_text = f"Technical issue: {str(e)}"
            with st.chat_message("assistant"):
